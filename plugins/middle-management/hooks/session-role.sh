...
if { [ "$ORCH_SRC" = marker ] && [ "$MY_ID" = "$ORCH_ID" ]; } \
  ...
  printf 'The same reader with --waiting lists sessions WAITING on their user: "permission" = parked on a\n'
  printf 'permission prompt, "question" = its last turn asks something. Such a session is alive but will\n'
  printf 'not move until its user acts in that tab: do not message it, tell your user which tab and why.\n'
